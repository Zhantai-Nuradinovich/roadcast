...
using roadcast.Infrastructure.Identity.Extensions;
...
public class AuthenticationService : IAuthenticationService
{
    ...
    public async Task AddClaimToUser(string email, string claimName, string claimValue)
    {
        ...
        if (!result.Succeeded)
        {
            throw new Exception($"Failed to add claim '{claimName}:{claimValue}' to user '{email}.");
        }
    ...
    public async Task AddUserToRoleAsync(string email, string roleName)
    {
        ...
        if (!result.Succeeded)
        {
            throw new Exception($"Failed to add role: {roleName} to user {email}.");
        }
    }
    ...
    public async Task<AppUser> GetUserInfoAsync(string userId)
    {
    ...
    public async Task RemoveUserFromRoleAsync(string email, string roleName)
    {
        ...
        if (!result.Succeeded)
        {
            throw new Exception($"Failed to remove role '{roleName}' from user '{email}'.");
        }
    }
}
